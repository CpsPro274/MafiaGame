...
export const StaggeredMenu = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const lastTime = layers.length
      ? (layers.length - 1) * 0.07
      : 0;

    const panelInsertTime =
      lastTime + (layers.length ? 0.08 : 0);
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current = gsap.to(all, {
      ...
      onComplete: () => {
        ...
        const socialTitle =
          panel.querySelector('.sm-socials-title');
  ...
  const renderLogo = () => {
    if (React.isValidElement(logoUrl)) {
      return logoUrl;
    }
